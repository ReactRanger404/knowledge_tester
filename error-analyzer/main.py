...
SYSTEM = """你是一个知识点讲解助手。请针对每道题目，无论对错，提取并讲解题目所考察的知识点。
...
async def handle(body: dict):
    ...
    text = "\n---\n".join(f"题号:{e.get('question_index','?')}\n题干:{e.get('stem','')}\n知识点:{e.get('knowledge_point','')}\n正确答案:{e.get('correct_answer','')}\n用户答案:{e.get('user_answer','')}\n反馈:{e.get('feedback','')}\n是否答对:{'是' if e.get('is_correct') else '否'}" for e in errors)
    r = await chat_structured([{"role":"system","content":SYSTEM},{"role":"user","content":f"以下题目请分析：\n{text}\n总题数：{total}"}], ErrorAnalysisReport)
